...
def predict(config):
    ...
    if prep_config["test_10crop"]:
        for i in range(10):
            dsets["database"+str(i)] = ImageList(config["dataset"], open(data_config["database"]["list_path"]).readlines(), \
            ...
            dsets["test"+str(i)] = ImageList(config["dataset"], open(data_config["test"]["list_path"]).readlines(), \
        ...
        dsets["database"] = ImageList(config["dataset"], open(data_config["database"]["list_path"]).readlines(), \
                                transform=prep_dict["database"])
        dset_loaders["database"] = util_data.DataLoader(dsets["database"], \
                                batch_size=data_config["database"]["batch_size"], \
                                shuffle=False, num_workers=4)
        dsets["test"] = ImageList(config["dataset"], open(data_config["test"]["list_path"]).readlines(), \
...
if __name__ == "__main__":
    ...
    if config["dataset"] == "imagenet":
        ...
        config["data"] = {"database":{"list_path":"../data/NUSWIDE-10/database.txt", "batch_size":16}, \
                          "test":{"list_path":"../data/NUSWIDE-10/test.txt", "batch_size":16}}
        config["R"] = 5000

    elif config["dataset"] == "NUSWIDE-21":
        config["data"] = {"database":{"list_path":"../data/NUSWIDE-21/database.txt", "batch_size":16}, \
                          "test":{"list_path":"../data/NUSWIDE-21/test.txt", "batch_size":16}}
        config["R"] = 5000
        ...
        config["data"] = {"database":{"list_path":"../data/flickr25k/database.txt", "batch_size":16}, \
                          "test":{"list_path":"../data/flickr25k/test.txt", "batch_size":16}}
        config["R"] = 5000
