...
def train(config):
    ...
    # 在 train 函数中
    dsets["train_set1"] = ImageList(config["dataset"],open(data_config["train_set1"]["list_path"]).readlines(), \
    ...
    dsets["train_set2"] = ImageList(config["dataset"],open(data_config["train_set2"]["list_path"]).readlines(), \
    ...
    for i in range(config["num_iterations"]):
        ...
        config["out_file"].write("Iter: {:05d}, loss: {:.3f}\n".format(i, similarity_loss.float().item()))
        config["out_file"].flush()  # 确保写入文件
...
if __name__ == "__main__":
    ...
    config["output_path"] = "../snapshot/" + config["dataset"] + "_" + str(config["hash_bit"]) + "bit_" + args.prefix
    ...
    config["loss"] = {
        ...
        "q_weight": 0,
        "l_threshold": 15.0,
        "sigmoid_param": 10.0 / config["hash_bit"],
        "class_num": args.class_num
    }

    if config["dataset"] == "imagenet":
        config["data"] = {
            "train_set1": {"list_path": "../data/imagenet/train.txt", "batch_size": 36},
            "train_set2": {"list_path": "../data/imagenet/train.txt", "batch_size": 36}
        }
    elif config["dataset"] == "NUSWIDE-10":
        config["data"] = {
            "train_set1": {"list_path": "../data/NUSWIDE-10/train.txt", "batch_size": 36},
            "train_set2": {"list_path": "../data/NUSWIDE-10/train.txt", "batch_size": 36}
        }
    elif config["dataset"] == "NUSWIDE-21":
        config["data"] = {
            "train_set1": {"list_path": "../data/NUSWIDE-21/train.txt", "batch_size": 36},
            "train_set2": {"list_path": "../data/NUSWIDE-21/train.txt", "batch_size": 36}
        }
    elif config["dataset"] == "coco":
        config["data"] = {
            "train_set1": {"list_path": "../data/coco/train.txt", "batch_size": 36},
            "train_set2": {"list_path": "../data/coco/train.txt", "batch_size": 36}
        }
    
    # elif config["dataset"] == "flickr25k":
    #     config["data"] = {
    #         "train_set1": {"list_path": "../data/flickr25k/mirflickr/train.txt", "batch_size": 36},
    #         "train_set2": {"list_path": "../data/flickr25k/mirflickr/train.txt", "batch_size": 36}
    #     }

    elif config["dataset"] == "flickr25k":
        config["data"] = {
        "train_set1": {"list_path": "../data/flickr25k/train.txt", "batch_size": 36},
        "train_set2": {"list_path": "../data/flickr25k/train.txt", "batch_size": 36}
    }
